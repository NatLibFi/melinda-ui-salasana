...
export function handleFailedQueryParams(req, res, next) {
  ...
  const failedParams = [...checkSalasanaQueryParams(queryParams)]
  ...
  //-----------------------------------------------------------------------------
  // Checks Salasana app specific query parameters
  //  - possible Salasana query parameters are: 
  //  - not all parameters are present in every request
  //
  function checkSalasanaQueryParams(queryParams) {
    return [];
  }
